...
arise when dispatching. First, the AUV can be at a location ({\em vent}) or be
...
the scientist will want the AUV to survey and sample a location and, if needed, direct the AUV on the fly to a new
...
to half an hour to go from {\em Surface} to {\em vent1},  five minutes
...
early in order to {\em sample vent2} so that the scientist can have the possibility to 
for more exploration. Conversely, heading back to the surface early would waste valuable
time if the scientist decide they want to {\em survey} another location. By 7pm though, it should
go to the surface so the scientist can pick it up by 8pm. In this scenario,
we see that the AUV did alternate between deciding to execute actions

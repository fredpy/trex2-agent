...
search all the tokens connected to $T$, we use the $SelectPolicy$
function to determine which execution policy should be returned. Herein
rises the issue of having multiply defined policies.  The function
$SelectPolicy$ could select an execution policy by the highest priority in the set or use
additive operators to combine them. In our case, the
earliest start time will be returned because it is the only defined policy. 
Otherwise, latest start will be used, in
Algorithm \ref{alg:dispatch}, if the set of defined policies is empty and null
is returned.
%\kcomment{I think this entire para needs to be

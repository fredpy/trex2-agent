...
note that our example only uses two policies, and the latest start time is the default.
Therefore, this issue does not occur in our example.
...
determines which policy out of the list should be used. Here rises the issue of having multiple policies.
$SelectPolicy$ could determine the policy in a number
of ways. Some examples are by using a priority for each policy or having additive operators for the policies.
In our case, the earliest start policy will be returned because it is the only policy we define.
The default policy of latest start will be used if $T$ is not connected to any token with a defined policy. 

...
//          }
    ...
    void sendMessage(Message * msg, ImcAdapter * sender, std::string ip, int port)
    {
      std::cout << "send message to " << ip << ":" << port << std::endl;
      //msg->setTimeStamp();
    ...
    {
      boost::thread(sendMessage, &msg, &m_adapter, ip, port);

...
In order to explain our execution method, we first consider some basic definitions related to a plan
...
A \emph{token} is a property that holds within two flexible interval, or timepoints,
 which represents a state \cite{py10dup}. 
...
A policy can be, but not limited to, early start or latest start as referred to in our example.  Moreover, the
...
in which case we either derive a policy or use a default one. In our example, we default to a strict latest start time but any policy can 
be used. 
...
to be added to the agent's plan as the mission time

...
As presented in \cite{lemai-chenevier2004}, while most of the
architectures do handle goals emerging in the course of mission. This
is often an afterthought and is considered similarly to any other
unexpected situataion such as a plan execution failure. Even more so,
in he work presented by the author, the emergence of new goals is
actually triggering full replanning as against a simple plan
repair. 
...
Prominent work in this area is related to dispatchability in simple
temporal networks (STN) so efficient temporal propagation prior to
execution encapsulates appropriate planning constraints \cite{mus98a}.
To do so, the STN supporting is transformed into an all-pairs network
and stripped of unnecessary edges, resulting in a more compact
temporal network that lessens propagation cost. The role of the
executive is to select time-points within the current execution bounds
and propagate its value within the simplified network. However, the
executive does not directly address how to decide what value should be
set for a given time-point in the scope of possible values for
dispatch.  When dealing with least-commitment planning this decision
is usually deferred to the plan executive. For instance, in
\cite{mus98c}, the executive has two responsibilities: the selection
and scheduling of plan events for execution.%  The executive needs to be
...
% initial constraints.
In all the cases we found in he literature the executive uses 
exclusively a {\em proactive} execution policy. This is motivated by
the intuition that it will give more time in the future should an 
unexpected situation occur. While this assumption is valid in 
most cases, our example do demonstrate that it can lead to 
...
controllability with STNs with uncertainty (STNU). \cite{morris01}
proposes an algorithm that inserts \textbf{wait} constraints within
the temporal network that waits until an uncontrollable event occurs
during planning. \cite{gallien2006} also uses a similar approach to
reduce Makespan; but they do so in the context of an exogenous event
-- such as for example the duration of a navigation task which depends
on external factors -- where it is necessary to defer actions for
correct plan execution. Work in soft temporal constraints with
preferences \cite{khatib2001temporal} on dispatching has another
related approach with preferable start times. However as
\cite{bartak2002} states, they are plagued by poor performance; while
the complexity can be reduced to a more tractable (polynomial)
solution by specifying preferences as convex functions
\cite{rossi2006learning}, this is still cubic in relation to the
number of variables. Moreover the solutions proposed optimize the plan
{\em a-priori} reducing plan flexibility leading to execution time
failure, which is not suitable for real-world embedded systems such as
ours.
...
Our work is complementary to much of the above.  While we do not
explicitly deal with dynamic controllability, our approach is not
inconsistent with that work. Second, our focus is not on the
observation of events during execution but rather on the possibility
of \emph{new objectives} arising at any time, and to avoid doing
actions that are not ``urgent'' to preserve the context for those
objectives.  Similarly, temporal preference specification during
planning, which has the effect of limiting execution-time flexibility.
And finally, most of the effort above, has decoupled execution from
planning unlike our work, where planning is not only continuous, but
execution is intertwined. Our interest is in deferring temporal
decisions to the executive to adapt the plan to new objectives without
heavily relying on plan-repair or re-planning solutions that could
impact system reactivity.

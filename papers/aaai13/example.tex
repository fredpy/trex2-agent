...
\begin{figure}[!t]
  \centering
  \subfloat[\small An illustration of our domain.]{\label{fig:ex:graph}\includegraphics[width=0.45\columnwidth]{figs/auv_example}}
  \hfill \subfloat[\small Initial plan.] {\label{fig:ex:plan}\includegraphics[width=0.55\columnwidth]{figs/example_plan}}
  ...
  \caption{\small{(\ref{fig:ex:graph}) illustrates the domain 
     and beside it is the initial plan for the problem
      (\ref{fig:ex:plan}). The AUV is initially at {\em Surface} at 8:00 
      and the mission is expected to complete by 20:00 with the
...
Given the initial problem in Fig. \ref{fig:ex:graph}, the onboard planner 
...
generation is a reality.  Finding a balancing between executing a plan action
\emph{proactive}ly or waiting is the
dichotomy that the plan executive needs to resolve. Otherwise, the plan will suffer along
...
After analyzing the objectives, we note that these divergent policies are
...
Our approach revolves around marking mission
objectives with a policy depending on the 
exploration of the plan structure, during plan execution, to identify if
the action is linked to a goal -- in which case it need

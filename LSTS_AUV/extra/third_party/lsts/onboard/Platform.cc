...
namespace TREX
{
  ...
  namespace LSTS
  {
    ...
    LstsReactor(arg), m_send_new_ref(false)
    {
      m_firstTick = true;
      m_blocked = false;
      m_connected = true;
    ...
    Platform::handleTickStart()
    {
      ...
      //if( referenceObservations.empty() ) {
        ...
        if( !m_blocked && !m_goals_pending.empty() ) {
          ...
          if( "reference"==gname ) {
            syslog(log::info)<<"Processing next goal on reference "<<gpred;
      ...
      try
      {
        ...
        while ((msg = m_adapter.poll(0, false)) != NULL)
        {
          ...
          else if (msg->getId() == PlanControlState::getIdStatic())
          {
            ...
            m_send_new_ref = previous_pcstate !=  NULL && (previous_pcstate->state != PlanControlState::PCS_EXECUTING || previous_pcstate->plan_id != "trex_plan");
            /*if (previous_pcstate !=  NULL)
              syslog(log::warn) << "Previous pcstate.plan_id " << previous_pcstate->plan_id << " var " << m_send_new_ref;
            else
              syslog(log::warn) << "Previous pcstate null var " << m_send_new_ref;*/
            // only true if previously wasn't on trex_plan or wasn't running
            m_send_new_ref = (pcstate->state == PlanControlState::PCS_EXECUTING && pcstate->plan_id == "trex_plan") && m_send_new_ref;
            //syslog(log::warn) << "New pcstate.plan_id " << pcstate->plan_id << " var " << m_send_new_ref;
